...
function Particular_categories_schemes(){
  const location = useLocation();
  const {category} = location.state || {};
  const [schemes, setSchemes] = React.useState([]);
  React.useEffect(() => {
    axios.get(`https://backend-wq9h.onrender.com/api/v1/users/givepaticularsectorscheme/${category.name}`)
    .then(res => { 
...
export default Particular_categories_schemes;